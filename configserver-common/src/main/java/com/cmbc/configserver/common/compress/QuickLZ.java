 ...
 * @Time 21:09
 */
public class QuickLZ implements Compress{
    ...
    private final static int MIN_OFFSET = 2;
    private final static int UNCONDITIONAL_MATCH_LENGTH = 6;
    private final static int UNCOMPRESSED_END = 4;
    private final static int CWORD_LEN = 4;
    private final static int DEFAULT_HEADER_LENGTH = 9;
    ...
    public static byte[] compress(byte[] source, int level) {
        int src = 0;
        int dst = DEFAULT_HEADER_LENGTH + CWORD_LEN;
        long cword_val = 0x80000000L;
        int cword_ptr = DEFAULT_HEADER_LENGTH;
        ...
        byte[] d2;
        int fetch = 0;
        int last_match_start = (source.length - UNCONDITIONAL_MATCH_LENGTH - UNCOMPRESSED_END - 1);
        int lits = 0;
        ...
        if (src <= last_match_start)
            fetch = (int) fast_read(source, src, 3);

        while (src <= last_match_start) {
            if ((cword_val & 1) == 1) {
                if (src > 3 * (source.length >> 2) && dst > src - (src >> 5)) {
                    d2 = new byte[source.length + DEFAULT_HEADER_LENGTH];
                    write_header(d2, level, false, source.length, source.length + DEFAULT_HEADER_LENGTH);
                    System.arraycopy(source, 0, d2, DEFAULT_HEADER_LENGTH, source.length);
                    return d2;
                }
            ...
            if (level == 1) {
                ...
                if (cache == 0 && hash_counter[hash] != 0 && (src - o > MIN_OFFSET || (src == o + 1 && lits >= 3 && src > 3 && source[src] == source[src - 3] && source[src] == source[src - 2] && source[src] == source[src - 1] && source[src] == source[src + 1] && source[src] == source[src + 2]))) {
                ...
                for (k = 0; k < QLZ_POINTERS_3 && (c > k || c < 0); k++) {
                    o = hashtable[hash][k];
                    if ((byte) fetch == source[o] && (byte) (fetch >>> 8) == source[o + 1] && (byte) (fetch >>> 16) == source[o + 2] && o < src - MIN_OFFSET) {
                        m = 3;
