...
public class JdbcTemplate extends JdbcAccessor{
	 ...
	 * <p>Creates a default {@link org.springframework.jdbc.support.SQLErrorCodeSQLExceptionTranslator}
	 * for the specified DataSource if none set, or a
	 * {@link org.springframework.jdbc.support.SQLStateSQLExceptionTranslator} in case of no DataSource.
	 * @see #getDataSource()
	 */
	...
	public Object query(
		...
		Assert.notNull(rse, "ResultSetExtractor must not be null");
		if(ConfigServerLogger.isTraceEnabled())
            ConfigServerLogger.trace("Executing prepared SQL query");
	...
	public int[] batchUpdate(String sql, final BatchPreparedStatementSetter pss) throws DataAccessException {
		if (ConfigServerLogger.isTraceEnabled()) {
            ConfigServerLogger.trace("Executing SQL batch update [" + sql + "]");
		}
				...
				if (outParam.isReturnTypeSupported()) {
					...
					if (out instanceof ResultSet) {
						if (outParam.isResultSetSupported()) {
                            ...
                            ConfigServerLogger.info("Added default SqlReturnResultSet parameter named " + rsName);
						}
					}
					else {
						returnedResults.put(outParam.getName(), out);
	 ...
	 * @throws org.springframework.jdbc.SQLWarningException if not ignoring warnings
	 * @see org.springframework.jdbc.SQLWarningException
	 */
	protected void handleWarnings(Statement stmt) throws SQLException {
		if (isIgnoreWarnings()) {
			if (ConfigServerLogger.isDebugEnabled()) {
				SQLWarning warningToLog = stmt.getWarnings();
				while (warningToLog != null) {
                    ConfigServerLogger.debug("SQLWarning ignored: SQL state '" + warningToLog.getSQLState() + "', error code '" +
	 ...
	 * @throws org.springframework.jdbc.SQLWarningException in case of an actual warning to be raised
	 */
	 ...
	 * @see org.springframework.jdbc.core.SqlProvider
	 */
